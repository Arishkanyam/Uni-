...
class Player(object):
    def __init__(self):
        ...
        self.rotatedSurf = pygame.transform.rotate(self.img, self.angle)  # тело корабля на которое накладывается картинка
        self.rotatedRect = self.rotatedSurf.get_rect()
        self.rotatedRect.center = (self.x, self.y)
        self.cosin = math.cos(math.radians(self.angle + 90))  # вычисляем угол поворота
        self.sin = math.sin(math.radians(self.angle + 90))  # вычисляем угол поворота
        self.head = (self.x + self.cosin * self.w // 2, self.y - self.sin * self.h // 2)  # точка из которой вылетают снаряды

    def draw(self, win):  # отображение
        win.blit(self.rotatedSurf, self.rotatedRect)

    def turnLeft(self):  # поворот налево
        self.angle += player_radial_speed  # скорость поворота берем из config.py
        self.rotatedSurf = pygame.transform.rotate(self.img, self.angle)  # поворачиваем поверхность корабля
        ...
        self.cosin = math.cos(math.radians(self.angle + 90))  # вычисление угла (координаты)
        self.sin = math.sin(math.radians(self.angle + 90))  # вычисление угла (координаты)
        self.head = (self.x + self.cosin * self.w // 2, self.y - self.sin * self.h // 2)  # точка из которой вылетают снаряды
    ...
    def moveForward(self):  # Движение вперед
        ...
        self.x += self.cosin * self.distance  # перемещение по Х
        self.y -= self.sin * self.distance  # перемещение по У
        self.rotatedSurf = pygame.transform.rotate(self.img, self.angle)  # поворачиваем корабль
        self.rotatedRect = self.rotatedSurf.get_rect()  # создаем поверхность
        self.rotatedRect.center = (self.x, self.y)  # находим центральную координату
        self.cosin = math.cos(math.radians(self.angle + 90))  # считаем угол
        self.sin = math.sin(math.radians(self.angle + 90))
        self.head = (self.x + self.cosin * self.w // 2, self.y - self.sin * self.h // 2)  # точка появления снарядов

    def updateLocation(self):  #обновление перемещения
        if self.x > sw + 50:
            self.x = 0
