...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, asteroid):
        ...
        self.rect = pygame.Rect(asteroid.x, asteroid.y, asteroid.size, asteroid.size)  # создаем прямоугольник по размеру астероида
    ...
    def draw(self, win):  # отрисовка взрыва
        ...
        if self.index >= len(self.images):  # если анимацию закончилась, то удаляем объект
            ...
            self.image = pygame.transform.scale(self.image, (self.size, self.size))  # масштабируем изображение по размеру астероида
